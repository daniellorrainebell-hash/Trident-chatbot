...
| Type, glow, UI surfaces | `styles.css` (`:root` tokens) |
...
**Type** is Sora (labels and marks) paired with Manrope (statement and
contact). Both are embedded as data URIs, generated by `npm run build:fonts`
into `src/styles/fonts.css`. They cannot be linked: the piece ships as a
single self-contained file under a policy that blocks external hosts, and a
...
atmosphere). Raising a single shadow instead just makes type look blurry.
