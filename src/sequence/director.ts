...
import { WORKFLOW_LAYER_T } from '../scene/layout'
...
export function buildCinematic(cut: CutId): DirectorHandle {
  ...
  // The stack assembles around the die: glass plates settle into place and
  // their copper comes up to a faint idle, unlit but present.
  tl.to(S, { systems: 1, duration: B.shellSolid.dur * 1.5, ease: 'power3.out' }, B.shellEdges.at)
  ...
  // Flare each layer as the signal actually arrives on it.
  WORKFLOW_LAYER_T.forEach((tt, i) => {
    const t = B.packet.at + tt * B.packet.dur
    tl.to(S.flare, { [i]: 1, duration: 0.3, ease: 'power2.out' }, t - 0.15)
    tl.to(S.flare, { [i]: 0, duration: 1.0, ease: 'power2.in' }, t + 0.15)
  })
...
export function enterInteractive() {
  gsap.killTweensOf([S, S.cam, S.sub, S.flare])
  ...
  SERVICES.forEach((_, i) => {
    gsap.to(S.sub, { [i]: 1, duration: 1.1, delay: i * 0.08, ease: 'power2.out' })
